...
public class ProjectPropertyPage extends PropertyPage {
	...
	private void createLibraySection(Composite parent){
		...
		try{
			List<String> libs = PreferenceUtil.getProjectLibsList(((IProject) getElement()).getProject());		  
			for(String lib:libs){
				String lb = lib.trim();
				if(lb.length()>0){
					this.getViewContentProvider(libPathList).add(getItemByLib(lb));
				}
			}	
			libPathList.refresh();
		}catch(Exception e){}
	...
	private void handleAddExternalResource() {
		...
		dialog.setMessage("Select a lib folder");
